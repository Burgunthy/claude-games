...
export function casinoCardArt(rank: string, suit: string, hidden: boolean = false): string {
  if (hidden) {
    return [
      '╔══════╗',
      '║░░░░░░║',
...
export function casinoBorder(type: 'box' | 'header' | 'separator'): string {
  switch (type) {
    case 'box':
      return '╔═╗║╚═╝';
    case 'header':
...
export function casinoCardCompact(rank: string, suit: string): string {
  const symbols = {
    hearts: '♥',
    diamonds: '♦',
    clubs: '♣',
