...
export class ManiacAI extends BaseAI {
  private chaosFactor: number = 0.7 + Math.random() * 0.3; // 0.7-1.0
  private bluffFrequency: number = 0.45 + Math.random() * 0.2; // 45-65%
  private unpredictableLevel: number = Math.random(); // 0-1

  constructor(name: string = 'Maniac AI') {
    super({
      strategy: 'maniac' as AIStrategyType,
      name,
      avatar: '🔥',
