...
function findRegularStraight(cards: Card[]): Card[] | null {
  const uniqueValues = Array.from(new Set(cards.map(c => rankValue(c))))
    ...
    if (uniqueValues[i + 4] - uniqueValues[i] === 4) {
      ...
      for (const value of straightValues) {
        ...
        if (card && !straightCards.includes(card)) {
          ...
          const anotherCard = cards.find(c =>
            rankValue(c) === value && !straightCards.includes(c)
          );
          if (anotherCard) straightCards.push(anotherCard);
        }
      }
...
function evaluate5Cards(cards: Card[]): EvaluatedHand {
  ...
  if (pairs.length >= 2) {
    ...
    const kicker = sorted.find(c =>
      rankValue(c) !== highValue && rankValue(c) !== lowValue
    )!;
...
export function evaluateHand(holeCards: Card[], communityCards: Card[] = []): EvaluatedHand {
  ...
  let bestHand: EvaluatedHand = evaluate5Cards(combinations[0]);
...
export function determineWinner(
  players: Array<{ id: string; name: string; holeCards: Card[] }>,
  communityCards: Card[]
): Array<{ id: string; name: string; hand: EvaluatedHand }> {
...
export function calculateHandStrength(holeCards: Card[], communityCards: Card[]): number {
  ...
  if (communityCards.length === 0) {
    ...
    if (isConnected && isSuited) return 0.35;
...
export function checkDraw(holeCards: Card[], communityCards: Card[]): {
  ...
  for (const [, cards] of suitGroups) {
    if (cards.length === 4) hasFlushDraw = true;
    if (cards.length === 3) hasBackdoorFlush = true;
  }
  ...
  for (let i = 0; i < values.length - 3; i++) {
    if (values[i + 3] - values[i] === 4) {
      ...
      for (let v = values[i] + 1; v < values[i + 3]; v++) {
        if (!values.includes(v)) missing.push(v);
      }
      if (missing.length === 1) {
...
export function formatCard(card: Card, style: 'compact' | 'full' = 'compact'): string {
