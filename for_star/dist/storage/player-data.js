...
import { DEFAULT_PLAYER_DATA } from '../core/types.js';
...
export function updateHandStats(data, won, atShowdown, action, amountWon = 0) {
    const newData = { ...data };
    newData.handsPlayed++;
    if (won) {
        newData.handsWon++;
...
/** Claim daily bonus */
export function claimDailyBonus(data) {
    const now = new Date();
...
export function canClaimDailyBonus(data) {
...
export function getStartingChips(tier) {
    const bonus = {
        Rookie: 0,
        Amateur: 500,
        Pro: 1500,
