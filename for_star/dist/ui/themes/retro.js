...
export function retroCardArt(rank, suit, hidden = false) {
    if (hidden) {
        return [
            '+======+',
            '|######|',
...
export function retroBorder(type) {
    switch (type) {
        case 'box':
            return '+-+|++';
        case 'header':
...
export function retroCardCompact(rank, suit) {
    const symbols = {
        hearts: 'H',
        diamonds: 'D',
        clubs: 'C',
...
export function retroColorCode(color) {
    const codes = {
        red: 31,
        green: 32,
        yellow: 33,
