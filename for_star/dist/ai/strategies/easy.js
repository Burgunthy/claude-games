...
export class EasyAI extends BaseAI {
    ...
    decideAction(holeCards, communityCards, context) {
        ...
        this.addThought(ThoughtStage.PERCEIVE, `I have ${this.formatHand(holeCards)} (${handType}). ${communityCards.length > 0 ? `Board: ${this.formatHand(communityCards)}` : 'No community cards yet.'}`, '👀');
        // Analyze stage
        let actionReasoning = '';
        if (context.phase === 'preflop') {
            ...
            this.addThought(ThoughtStage.ANALYZE, `${actionReasoning} Position: ${this.formatPosition(context.position)}. Pot odds: ${(context.potOdds * 100).toFixed(0)}%`, '🧮');
        }
        else {
            // Postflop
            ...
            this.addThought(ThoughtStage.ANALYZE, `${actionReasoning} Hand strength: ${(handStrength.current * 100).toFixed(0)}%. Pot odds: ${(context.potOdds * 100).toFixed(0)}%`, '🧮');
        }
        // Decide stage
