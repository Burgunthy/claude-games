...
class AISimulation {
    ...
    errors = [];
    stats = new Map();
    constructor(strategies, startingChips = 1000) {
        // Create players
    ...
    buildContext(player) {
        ...
        if (playersAfterDealer <= 1)
            position = 'early';
        else if (playersAfterDealer >= totalActive - 2)
            position = 'late';
        return {
            phase: this.phase,
    ...
    printResults() {
        ...
        for (const player of this.players) {
            ...
            if (stats) {
                console.log(sprintf('%-12s %-8s %-6d %-6d %-6d %-6d %-6d %-10d', player.name, player.strategy, stats.folds, stats.checks, stats.calls, stats.raises, stats.allins, player.chips));
            }
        }
