...
export class HardAI extends BaseAI {
  private rangeTightness: number = 0.6 + Math.random() * 0.2;
  private aggressionFactor: number = 0.65 + Math.random() * 0.2;
  private bluffFrequency: number = 0.25 + Math.random() * 0.15;

  // Track opponent tendencies (simplified)
  private opponentAggression: Map<string, number> = new Map();

  constructor(name: string = 'Hard AI') {
    super({
      strategy: 'hard' as AIStrategyType,
      name,
      avatar: '🦈',
  ...
  private makeDecision(
    strength: HandStrength,
    context: GameContext,
    boardTexture: { type: string; wetness: number }
  ): Decision {
  ...
  private analyzeBoardTexture(cards: Card[]): { type: string; wetness: number } {
    if (cards.length === 0) {
    ...
    for (let i = 1; i < sortedRanks.length; i++) {
      if (sortedRanks[i] - sortedRanks[i - 1] <= 2) connectedCount++;
    }
    ...
    if (flushPossible) wetness += 0.3;
    if (straightPossible) wetness += 0.3;
    if (cards.length >= 3 && uniqueSuits === 1) wetness += 0.4; // Monotone

    // Paired board (less wet)
    const uniqueRanks = new Set(ranks).size;
    if (uniqueRanks < cards.length) wetness -= 0.2;

    // Type classification
    let type = 'Dry';
    if (wetness > 0.6) type = 'Very Wet';
    else if (wetness > 0.4) type = 'Wet';
    else if (wetness > 0.2) type = 'Medium';

    return { type, wetness: Math.max(0, Math.min(1, wetness)) };
  }

  private shouldBluffBasedOnSituation(context: GameContext, boardTexture: { type: string; wetness: number }): boolean {
    // Fewer players = more bluffing
    if (context.playersInHand > 2) return Math.random() < this.bluffFrequency * 0.3;
  ...
  private calculateBetSize(pot: number, phase: GamePhase, type: 'value' | 'bluff'): number {
    const baseMultipliers: Record<GamePhase, number> = {
      preflop: 2.5,
      flop: 0.65,
      turn: 0.75,
      river: 0.8,
  ...
  private assessRangeAdvantage(context: GameContext, communityCards: Card[]): number {
    // Simplified range advantage calculation
    // In a real implementation, this would track opponent ranges
    if (context.position === 'late') return 0.15;
    if (context.position === 'early') return -0.1;
    return 0;
  }

  private getDrawEquity(strength: HandStrength, _communityCount: number): number {
    if (strength.drawType === 'both') return 0.45;
    if (strength.drawType === 'flush') return _communityCount === 3 ? 0.35 : 0.2;
    if (strength.drawType === 'straight') return _communityCount === 3 ? 0.32 : 0.18;
    return 0.25;
  }

  private classifyHand(holeCards: Card[], communityCards: Card[]): string {
    if (communityCards.length === 0) {
  ...
  private classifyHoleCards(cards: Card[]): string {
    ...
    if (isConnected && isSuited && values[0] >= 10) return 'Premium suited connectors';
    if (isConnected && isSuited) return 'Speculative suited connectors';
    if (isSuited && values[0] >= 10) return 'Suited high cards';
