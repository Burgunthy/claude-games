...
import { Player, SidePot, PotResult } from './types.js';

/** Calculate main pot and side pots for all-in situations */
export function calculatePots(activePlayers: Player[]): PotResult {
...
export function getCallAmount(currentBet: number, playerBet: number, playerChips: number): number {
...
export function isValidRaise(
  ...
  if (totalBet > playerBet + playerChips) return false; // Can't raise more than you have

  return true;
}

/** Calculate pot odds */
export function getPotOdds(pot: number, toCall: number): number {
  if (toCall === 0) return 1; // No cost to call
  return pot / toCall;
}

/** Calculate expected value of a call */
export function getCallEV(pot: number, toCall: number, winProbability: number): number {
  if (toCall === 0) return pot * winProbability;
